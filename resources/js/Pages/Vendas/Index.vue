<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex items-center justify-between mb-8">
      ...
      <a v-if="can('vendas.criar')" :href="route('vendas.create')" class="px-6 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-colors">
        + Nova Venda
      </a>
    </div>
            ...
            <tr v-for="v in (data?.data || [])" :key="v.id" class="border-b last:border-b-0 hover:bg-gray-50">
              ...
              <td class="p-3 text-center">
                <div class="flex justify-center gap-3">
                  <a v-if="can('vendas.ver')" :href="`/vendas/${v.id}`" class="text-indigo-600 hover:text-indigo-800 font-medium">Visualizar</a>
                  <a v-if="can('vendas.editar')" :href="`/vendas/${v.id}/edit`" class="text-green-600 hover:text-green-800 font-medium">Editar</a>
                  <a v-if="can('vendas.ver')" :href="`/vendas/${v.id}/recibo`" target="_blank" class="text-purple-600 hover:text-purple-800 font-medium">Recibo</a>
